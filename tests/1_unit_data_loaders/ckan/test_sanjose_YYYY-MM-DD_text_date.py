...
source = 'San Jose'
table = defs.TableType.CALLS_FOR_SERVICE
year = 2016
month = 2
...
def jsondata(check_for_dataset, row):
    ...
    where = '"' + row['date_field'] + '"' + rf" LIKE '%{year}-{month:02}%'"
...
def test_get_count_date_filter(check_for_dataset, gt, row, loader):
    ...
    date = [f'{year}-{month:02}-02',f'{year}-{month:02}-05']
    dts = [pd.to_datetime(x) for x in date]
    gt_dts = pd.to_datetime(gt[row['date_field']])
