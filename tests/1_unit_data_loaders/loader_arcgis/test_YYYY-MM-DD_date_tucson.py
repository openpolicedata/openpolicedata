...
def jsondata(check_for_dataset, row):
    ...
    r = requests.get(url, {'f':'json','where':'1=1','outFields':'*', 'orderByFields': f'{row["date_field"]}, OBJECTID_1'})
    r.raise_for_status()
...
@pytest.mark.parametrize('year', [2013, [2013,2014]])
...
@pytest.mark.parametrize('date', [2014, ['2012-06-01','2013-06-01']])
@pytest.mark.parametrize('nrows', [None, 2])
@pytest.mark.parametrize('offset', [0, 1])
def test_load(check_for_dataset, gt, row, loader, date, nrows, offset):
    if not check_for_dataset(source, table):
        return
