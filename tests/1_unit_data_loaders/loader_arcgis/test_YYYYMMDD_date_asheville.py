...
def jsondata(check_for_dataset, row):
    ...
    r = requests.get(url, {'f':'json','where':'1=1','outFields':'*', 'orderByFields': f'{row["date_field"]}, OBJECTID'})
    r.raise_for_status()
...
def test_get_count_year_filter(check_for_dataset, gt, gt_raw, loader, row, year):
    ...
    if count != is_year.sum():
        # There is one date that is MMDDYYYY instead of YYYYMMDD, not sure there is anything that can be done unfortunately
        is_year = pd.Series(False, index=gt_raw.index)
        for y in year:
            is_year = is_year | gt_raw[row['date_field']].str.startswith(str(y))
...
@pytest.mark.parametrize('date', [2023, ['2022-06-01',2022]])
