...
#TODO: Get tests to pass by modifying changed_rows_fixture settings
class TestDatasets:
    @pytest.mark.parametrize("file", [(), (csv_file,)])
    def test_reload(self, file):
    ...
    def test_duplicates(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile, use_changed_rows=use_changed_rows)
        assert not datasets.duplicated(subset=['State', 'SourceName', 'Agency', 'TableType','Year']).any()

    def test_check_columns(self, csvfile, use_changed_rows):
        columns = {
    ...
    def test_table_for_nulls(self, csvfile, use_changed_rows):
    ...
    def test_check_state_names(self, csvfile, use_changed_rows):
        all_states = [
    ...
    def test_agency_names(self, csvfile, use_changed_rows):
    ...
    def test_year(self, csvfile, use_changed_rows):
    ...
    def test_dataset_id(self, csvfile, data_type,use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
        rem = datasets["dataset_id"][datasets["DataType"] == data_type]
        assert pd.isnull(rem).sum() == 0

    def test_years_multi(self, csvfile, skip, use_changed_rows):
        if skip != None:
            skip = skip.split(",")
    ...
    def test_agencies_multi(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
        rem = datasets["agency_field"][datasets["Agency"] == opd.defs.MULTI]
        assert pd.isnull(rem).sum() == 0

    def test_arcgis_urls(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
        urls = datasets["URL"]
    ...
    def test_source_list_by_state(self, csvfile ):
        datasets = get_datasets(csvfile,use_changed_rows=False)
        state = "Virginia"
    ...
    def test_source_list_by_agency(self, csvfile ):
        datasets = get_datasets(csvfile,use_changed_rows=False)
    ...
    def test_table_types(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
        for t in datasets["TableType"]:
            # Try to convert to an enum
            opd.defs.TableType(t)

    def test_data_types(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
        for t in datasets["DataType"].unique():
            # Try to convert to an enum
            opd.defs.DataType(t)

    def test_min_versions(self, csvfile, use_changed_rows):
        datasets = get_datasets(csvfile,use_changed_rows=use_changed_rows)
